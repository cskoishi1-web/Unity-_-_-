Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/de90c146b65d851448f0b80e63b4df67_-1631155487070029365" {
	Properties {
		...
		[ThryToggle(POI_BUMP2NDMAP)] [HideInInspector] _UseBump2ndMap ("Enable", Float) = 0
		...
		[ThryHideInInspector] [ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9, Screen Space, 10)] _Bump2ndMapUV ("UV", Float) = 0
		...
		[ThryHideInInspector] [ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9, Screen Space, 10)] _Bump2ndScaleMaskUV ("UV", Float) = 0
		[ThryWideEnum(Off, 0, 1R, 1, 1G, 2, 1B, 3, 1A, 4, 2R, 5, 2G, 6, 2B, 7, 2A, 8, 3R, 9, 3G, 10, 3B, 11, 3A, 12, 4R, 13, 4G, 14, 4B, 15, 4A, 16)] _Bump2ndScaleMaskGlobalMask ("Global Mask--{reference_property:_Bump2ndScaleMaskGlobalMaskBlendType}", Float) = 0
		[ThryHideInInspector] [ThryWideEnum(Add, 7, Subtract, 1, Multiply, 2, Divide, 3, Min, 4, Max, 5, Average, 6, Replace, 0)] _Bump2ndScaleMaskGlobalMaskBlendType ("Blending", Float) = 2
		...
		[Helpbox(1, 2, 1)] _Decal1PosHelp ("To apply Positioning using Raycast, edit this Material directly from your Mesh in the Scene. Click for video tutorial.--{onClick:{type:URL,data:https://www.poiyomi.com/color-and-normals/decals#positioning}}", Float) = 0
		...
		[ThryWideEnum(Off, 0, Shrink to Fit, 1, Grow to Fit, 2)] _Decal1AspectRatioMode ("Aspect Ratio", Float) = 0
		...
		[ThryHideInInspector] [ThryWideEnum(Off, 0, Theme Color 0, 1, Theme Color 1, 2, Theme Color 2, 3, Theme Color 3, 4, ColorChord 0, 5, ColorChord 1, 6, ColorChord 2, 7, ColorChord 3, 8, AL Theme 0, 9, AL Theme 1, 10, AL Theme 2, 11, AL Theme 3, 12)] _DecalColor1ThemeIndex ("", Float) = 0
		...
		[ThryHideInInspector] [ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9, Screen Space, 10)] _DecalTexture1UV ("UV", Float) = 0
		[ThryHideInInspector] [Enum(R, 0, G, 1, B, 2, A, 3)] _Decal1MaskChannel ("Mask Channel", Float) = 1
		...
		[ThryHideInInspector] [Enum(Everywhere, 0, Decal Bounds, 1, Everywhere Masked, 2, Bounds Masked, 3)] _Decal1OverrideAlphaMode ("Mode", Float) = 0
		_DecalBlendAlpha1 ("Alpha", Range(0, 1)) = 1
		_DecalAlphaIntensity1 ("Alpha Intensity", Range(0, 10)) = 1
		[ToggleUI] _Decal1UseVideoEffects ("Apply Video Effect--{condition_showS:_VideoEffectsEnable==1}", Float) = 1
		_Decal1Depth ("Depth", Range(-0.5, 2)) = 0
		[HideInInspector] s_start_Decal1ColorAdjust ("Color Adjust--{reference_property:_DecalColorAdjustEnabled1, persistent_expand:true, default_expand:false}", Float) = 0
		[ThryToggleUI(true)] [HideInInspector] _DecalColorAdjustEnabled1 ("Color Adjust", Float) = 0
		_DecalSaturation1 ("Saturation", Range(-1, 10)) = 0
		_DecalChromatize1 ("Chromatize", Range(-1, 10)) = 0
		_DecalBrightness1 ("Brightness", Range(-1, 2)) = 0
		_DecalGamma1 ("Gamma", Range(0.01, 5)) = 1
		[HideInInspector] ss_start_Decal1HueShift ("Hue Shift--{reference_property:_DecalHueShiftEnabled1, persistent_expand:true, default_expand:false}", Float) = 0
		...
		[HideInInspector] ss_end_Decal1HueShift ("Hue Shift", Float) = 0
		[HideInInspector] s_end_Decal1ColorAdjust ("Color Adjust", Float) = 0
		...
		[ThryHideInInspector] [ToggleUI] _Decal1VideoEnabled ("Video Texture", Float) = 0
		...
		[ThryHideInInspector] [ThryWideEnum(Add, 7, Subtract, 1, Multiply, 2, Divide, 3, Min, 4, Max, 5, Average, 6, Replace, 0)] _Decal1GlobalMaskBlendType ("Blending", Range(0, 1)) = 2
		...
		[ThryHideInInspector] [ThryWideEnum(Add, 7, Subtract, 1, Multiply, 2, Divide, 3, Min, 4, Max, 5, Average, 6, Replace, 0)] _Decal1ApplyGlobalMaskBlendType ("Blending", Float) = 0
		[ToggleUI] _Decal1OnOutlines ("Show on Outlines", Float) = 0
		...
		[Space(7)] [HideInInspector] s_start_Decal1ALColorOverrideS ("Color Override--{reference_property:_AudioLinkDecal1ColorOverrideEnabled, persistent_expand:true, default_expand:false}", Float) = 0
		[ThryHideInInspector] [ToggleUI] _AudioLinkDecal1ColorOverrideEnabled ("Color Override", Float) = 0
		_AudioLinkDecal1ColorOverride ("Color", Vector) = (1,1,1,1)
		[ThryWideEnum(Off, 0, Theme Color 0, 1, Theme Color 1, 2, Theme Color 2, 3, Theme Color 3, 4, ColorChord 0, 5, ColorChord 1, 6, ColorChord 2, 7, ColorChord 3, 8, AL Theme 0, 9, AL Theme 1, 10, AL Theme 2, 11, AL Theme 3, 12)] _AudioLinkDecal1ColorOverrideThemeIndex ("Theme Color", Float) = 0
		[Enum(Bass, 0, Low Mid, 1, High Mid, 2, Treble, 3, Volume, 4)] _AudioLinkDecal1ColorOverrideBand ("Color Override Band", Float) = 0
		[Ramp4(normalized)] _AudioLinkDecal1ColorOverrideRamp ("Color Override Ramp", Vector) = (0,1,0,1)
		[HideInInspector] s_end_Decal1ALColorOverrideS ("", Float) = 0
		...
		[ThryToggle(_CUBEMAP)] [HideInInspector] _CubeMapEnabled ("Enable CubeMap", Float) = 0
		[ThryWideEnum(Skybox, 0, Reflection, 1, World Normal Direction, 2, Local Normal Direction, 3)] _CubeMapUVMode ("UV Mode", Float) = 1
		_CubeMapWorldNormalsStrength ("Normals Strength--{condition_showS:(_CubeMapUVMode==2)}", Range(0, 1)) = 1
		_CubeMapColor ("Color--{reference_property:_CubeMapColorThemeIndex}", Vector) = (1,1,1,1)
		[ThryHideInInspector] [ThryWideEnum(Off, 0, Theme Color 0, 1, Theme Color 1, 2, Theme Color 2, 3, Theme Color 3, 4, ColorChord 0, 5, ColorChord 1, 6, ColorChord 2, 7, ColorChord 3, 8, AL Theme 0, 9, AL Theme 1, 10, AL Theme 2, 11, AL Theme 3, 12)] _CubeMapColorThemeIndex ("", Float) = 0
		[NoScaleOffset] _CubeMap ("CubeMap--{reference_properties:[_CubeMapRotation, _CubeMapRotationPan]}", Cube) = "" {}
		[Vector3] [HideInInspector] _CubeMapRotation ("Rotation in Degrees", Vector) = (0,0,0,0)
		[Vector3] [HideInInspector] _CubeMapRotationPan ("Panning in Degrees", Vector) = (0,0,0,0)
		...
		[Enum(Vertex, 0, Pixel, 1)] _CubeMapNormal ("Normal to use", Float) = 1
		...
		[ThryHideInInspector] [Vector2] _CubeMapMaskPan ("Panning", Vector) = (0,0,0,0)
		[ThryHideInInspector] [ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9, Screen Space, 10)] _CubeMapMaskUV ("UV", Float) = 0
		[ThryHideInInspector] [Enum(R, 0, G, 1, B, 2, A, 3)] _CubeMapMaskChannel ("Channel", Float) = 0
		[ThryHideInInspector] [ToggleUI] _CubeMapMaskInvert ("Invert", Float) = 0
		_CubeMapLightMask ("Hide in Shadow", Range(0, 1)) = 0
		[ThryWideEnum(Off, 0, 1R, 1, 1G, 2, 1B, 3, 1A, 4, 2R, 5, 2G, 6, 2B, 7, 2A, 8, 3R, 9, 3G, 10, 3B, 11, 3A, 12, 4R, 13, 4G, 14, 4B, 15, 4A, 16)] _CubeMapMaskGlobalMask ("Global Mask--{reference_property:_CubeMapMaskGlobalMaskBlendType}", Float) = 0
		[ThryHideInInspector] [ThryWideEnum(Add, 7, Subtract, 1, Multiply, 2, Divide, 3, Min, 4, Max, 5, Average, 6, Replace, 0)] _CubeMapMaskGlobalMaskBlendType ("Blending", Range(0, 1)) = 2
		...
		[ThryHideInInspector] [ToggleUI] _CubeMapHueShiftEnabled ("Enabled", Float) = 0
		[ThryWideEnum(OKLab, 0, HSV, 1)] _CubeMapHueShiftColorSpace ("Color Space", Float) = 0
		[ThryWideEnum(Hue Select, 0, Hue Shift, 1)] _CubeMapHueSelectOrShift ("Select or Shift", Float) = 1
		...
		[ThryToggle(POI_ENVIRORIM)] [HideInInspector] _EnableEnvironmentalRim ("Enable", Float) = 0
		...
		[ThryHideInInspector] [ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9, Screen Space, 10)] _RimEnviroMaskUV ("UV", Float) = 0
