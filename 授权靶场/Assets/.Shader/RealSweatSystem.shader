Shader "RealSweatSystem" {
	Properties {
		[Header(00  TESSELLATION SILHOUETTE)] [Toggle] _TessellationEnable ("Tessellation Silhouette (Off = Flat / Lowest)", Float) = 1
		[Enum(Simplified,2.5,Balanced,4,Precise,7)] _TessellationFactor ("3D Detail Precision", Float) = 2.5
		[Toggle] _SparseTessellation ("Drop-Aware Sparse Tessellation", Float) = 1
		_TessFadeDistance ("Tessellation Fade Distance (m)", Range(0.5, 10)) = 3.5
		[HideInInspector] _TessTargetEdgePixels ("Tessellation Target Edge Pixels", Range(4, 32)) = 10
		_DisplacementHeight ("Droplet Silhouette Height (m)", Range(0, 0.01)) = 0.00135
